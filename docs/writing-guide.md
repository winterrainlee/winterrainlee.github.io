...
제목이 있는 콜아웃에서 본문을 여러 줄로 쓸 때는 제목 줄 다음에 빈 인용 줄 `>`을 하나 두고, 본문 줄도 모두 `>`로 시작하는 방식을 권장한다. 특히 성경 구절처럼 순서가 중요한 인용문은 이 형태가 가장 안전하다.

```md
> [!quote] 이사야 2:11-17
>
> 11 그날에 눈이 높은 자가 낮아지며 ...
> 12 대저 만군의 여호와의 한 날이 ...
> 13 또 레바논의 높고높은 모든 백향목과 ...
```

아래처럼 제목 줄만 `>`로 쓰고 다음 줄부터 일반 문장처럼 쓰는 방식은 Markdown의 lazy continuation 규칙 때문에 빌드 결과가 예상과 다르게 묶일 수 있다.
...
- 다른 글 링크가 모호하지 않은가?
- 제목이 있는 콜아웃의 여러 줄 본문은 각 줄이 `>`로 시작하는가?

블로그 매니저에게 초안 발행을 맡길 때는 매니저가 이 가이드를 기준으로 구조를 함께 점검한다. 본문 표현은 임의로 고치지 않지만, 다음 항목은 발행 계획에 포함해 제안할 수 있다.

- 관련 글로 이어지는 내부 링크 추가 또는 표시 문구 정리
- `[[memo/first-note|첫번째 메모]]`처럼 모호하지 않은 내부 링크 형식 제안
- 제목 있는 콜아웃의 줄 구조 정리
- 성경 구절, 인용문, 목록처럼 빌드 후 순서와 묶임이 중요해 보이는 부분의 Markdown 구조 점검
